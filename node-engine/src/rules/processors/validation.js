...
class RuleValidation {
  ...
  async validateItemCondition(item, condition) {
    ...
    try {
      ...
      if (condition.condition === "item.column === 'New'" || condition.condition === 'item.column === "New"') {
      ...
      if (condition.condition === "item.column === 'Done'" || condition.condition === 'item.column === "Done"') {
        const result = item.column === 'Done';
        log.debug(`Column check (Done): ${item.column} === 'Done' -> ${result}`);
        return result;
      }

      if (condition.condition === "item.column === 'Waiting'" || condition.condition === 'item.column === "Waiting"') {
      ...
      if (condition.condition === 'item.column === item.pr.column && item.assignees === item.pr.assignees') {
        const result =
          item.column === item.pr?.column && JSON.stringify(item.assignees) === JSON.stringify(item.pr?.assignees);
