...
describe('Column Transition Enforcement in columns.js', () => {
  ...
  test('validateColumnTransition should allow valid transition from New to Active', async () => {
    ...
    assert.strictEqual(result.valid, true, 'New -> Active transition should be allowed (valid per rules.yml)');
  });
  ...
  test('validateColumnTransition should allow valid transition from None to Active', async () => {
    ...
    assert.strictEqual(result.valid, true, 'None -> Active transition should be allowed (initial column setting)');
  });
  ...
  test('validateColumnTransition should block invalid transition from Active to New', async () => {
    ...
    assert.strictEqual(result.valid, false, 'Active -> New transition should be blocked (not in validTransitions)');
    assert.ok(result.reason, 'Should provide a reason for blocking');
    assert.ok(
      result.reason.includes('not allowed') || result.reason.includes('No transitions defined'),
  ...
  test('validateColumnTransition should handle validation errors and throw', async () => {
    ...
    try {
      ...
      assert.ok(
        error.message.includes('validation') || error.message.includes('transition'),
  ...
  test('validateColumnTransition should handle same column transition', async () => {
    ...
    assert.strictEqual(result.valid, true, 'Same column transition should be allowed (no change)');
  });
  ...
  test('validateColumnTransition should handle null/undefined columns', async () => {
    ...
    assert.strictEqual(nullResult.valid, true, 'null -> Active should be allowed (initial setting)');
    assert.strictEqual(undefinedResult.valid, true, 'undefined -> Active should be allowed (initial setting)');
  });
});
