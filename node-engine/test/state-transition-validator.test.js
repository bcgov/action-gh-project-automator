...
import { StateTransitionValidator } from '../src/utils/state-transition-validator.js';

describe('StateTransitionValidator', () => {
  test('validates simple column transitions', async () => {
    ...
    // Note: addColumnTransitionRule marks RULES_VALIDATED
    ...
    const noReviewersResult = validator.validateColumnTransition('Active', 'Review', { item: { hasReviewers: false } });
    assert.equal(noReviewersResult.valid, false, 'Should reject when conditions not met');

    const hasReviewersResult = validator.validateColumnTransition('Active', 'Review', { item: { hasReviewers: true } });
    assert.equal(hasReviewersResult.valid, true, 'Should allow when conditions met');
  });

  test('validates complete state transitions', async () => {
    const validator = new StateTransitionValidator();
    ...
    assert.ok(invalidResult.errors[0].includes('Cannot remove assignees'), 'Should explain assignee removal error');
