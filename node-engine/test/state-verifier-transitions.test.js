...
describe('StateVerifier transition initialization', () => {
  test('StateVerifier.initializeTransitionRules loads validTransitions and enforces them', () => {
    ...
    assert.ok(disallowed.reason.includes('not allowed'), 'Blocked transition should provide clear reason');
  });
  ...
  test('StateVerifier.initializeTransitionRules replaces previously loaded transitions', () => {
    ...
    StateVerifier.initializeTransitionRules(buildRules([{ from: 'New', to: 'Active', conditions: [] }]));
    ...
    StateVerifier.initializeTransitionRules(buildRules([{ from: 'New', to: 'Review', conditions: [] }]));
