...
class RuleValidation {
  ...
  async validateItemCondition(item, condition) {
    ...
    try {
      ...
      if (condition.type) {
        ...
        if (!typeMatch) {
          log.debug(
            `Type mismatch: ${item.__typename} does not match ${JSON.stringify(condition.type)}`
          );
          return false;
        }
      ...
      if (condition.condition === 'monitored.repos.includes(item.repository)') {
        ...
        log.debug(
          `Repository check: ${item.repository?.nameWithOwner} in monitored repos -> ${result}`
        );
        return result;
      }
      ...
      if (
        ...
        const result =
          item.assignees?.nodes?.some((a) => this.monitoredUsers.has(a.login)) || false;
        log.debug(
          `Monitored users assignee check: ${item.assignees?.nodes?.map((a) => a.login).join(', ')} includes monitored user -> ${result}`
        );
        return result;
      }
      ...
      if (
        condition.condition ===
          "item.column === 'New' || item.column === 'Parked' || item.column === 'Backlog'" ||
        condition.condition ===
          'item.column === "New" || item.column === "Parked" || item.column === "Backlog"'
      ) {
        const result =
          item.column === 'New' || item.column === 'Parked' || item.column === 'Backlog';
        log.debug(
          `Column check (inactive): ${item.column} in ['New', 'Parked', 'Backlog'] -> ${result}`
        );
        return result;
      }
      ...
      if (condition.condition === 'item.assignees.includes(item.author)') {
        ...
        log.debug(
          `Author assignee check: ${item.assignees?.nodes?.map((a) => a.login).join(', ')} includes ${item.author?.login} -> ${result}`
        );
        return result;
      }
  ...
  async validateSkipRule(item, skipIf) {
    ...
    try {
      ...
      if (skipIf === "item.sprint === 'current'" || skipIf === 'item.sprint === "current"') {
        const result = item.sprint === 'current';
      ...
      if (skipIf === 'item.assignees.includes(item.author)') {
        const result = item.assignees?.nodes?.some((a) => a.login === item.author?.login) || false;
        log.debug(
          `Skip check (author assigned): ${item.assignees?.nodes?.map((a) => a.login).join(', ')} includes ${item.author?.login} -> ${result}`
        );
        return result;
      }
