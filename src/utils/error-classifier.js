...
export function classifyError(error) {
  ...
  const isAuthError =
    errorMessageLower.includes('bad credentials') ||
  ...
  const isNetworkError =
    (errorCode && networkErrorCodes.includes(errorCode)) || errorMessageLower.includes('timeout');
...
export function handleClassifiedError(error, itemIdentifier, log) {
  ...
  if (classification.isAuthError || classification.isRateLimitError) {
    const apiError = new Error(
      `GitHub API error: ${error.message}. Please check configuration and retry.`
    );
