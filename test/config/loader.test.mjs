...
test('ConfigLoader', async (t) => {
  await t.test('loads valid config from rules.yml', async () => {
    ...
    assert.ok(
      Array.isArray(config.automation.user_scope.monitored_users),
      'monitored_users is an array'
    );
    assert.ok(
      config.automation.user_scope.monitored_users.length > 0,
      'has at least one monitored user'
    );
    assert.ok(
      config.automation.user_scope.monitored_users.includes('DerekRoberts'),
      'includes DerekRoberts'
    );
    ...
    assert.ok(
      Array.isArray(config.automation.repository_scope.repositories),
      'has repository list'
    );
    assert.ok(
      config.automation.repository_scope.repositories.includes('bcgov/nr-nerds'),
      'includes current repo with org'
    );
