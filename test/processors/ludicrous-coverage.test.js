...
describe('RuleValidation - Ludicrous Coverage', () => {
  ...
  describe('Condition Whitelist', () => {
    ...
    const conditions = [
      ...
      {
        cond: 'monitored.users.includes(item.author)',
        item: { author: { login: 'other' } },
        expected: false,
      },
      { cond: 'monitored.repos.includes(item.repository)', expected: true },
      {
        cond: 'monitored.repos.includes(item.repository)',
        item: { repository: { nameWithOwner: 'other/repo' } },
        expected: false,
      },
      ...
      { cond: 'item.column === "New"', expected: true },
      {
        cond: "item.column === 'Next' || item.column === 'Active'",
        item: { column: 'Next' },
        expected: true,
      },
      {
      ...
      {
        cond: "item.column === 'New' || item.column === 'Parked' || item.column === 'Backlog'",
        item: { column: 'Parked' },
        expected: true,
      },
      ...
      {
        cond: 'item.column === item.pr.column && item.assignees === item.pr.assignees',
        expected: true,
      },
    ...
    conditions.forEach(({ cond, item: overrideItem, expected }) => {
      test(`condition "${cond}" should return ${expected}`, async () => {
        const result = await validator.validateItemCondition(overrideItem || item, {
          condition: cond,
        });
        assert.equal(result, expected);
      });
