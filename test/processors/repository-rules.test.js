...
describe('PR/Issue from monitored repository rule', () => {
  ...
  test('adds PR to board when from monitored repository', async () => {
    ...
    assert.ok(
      logMessages.some((msg) =>
        msg.includes('Rule Items from Repository triggered for PullRequest #123')
      )
    );
  });
  ...
  test('adds Issue to board when from monitored repository', async () => {
    ...
    assert.ok(
      logMessages.some((msg) => msg.includes('Rule Items from Repository triggered for Issue #456'))
    );
  });
