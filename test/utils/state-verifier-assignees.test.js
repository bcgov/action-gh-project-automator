...
test('verifyAssignees with real data (dry run)', async (t) => {
  ...
  await t.test('should handle real project data correctly', async (st) => {
    ...
    st.mock.method(githubApi.octokit, 'graphql', () =>
      Promise.reject(new Error('Could not resolve project'))
    );
    ...
    assert.ok(
      typeof StateVerifier.verifyAssignees === 'function',
      'verifyAssignees should be a function'
    );
  ...
  await t.test('should validate function signature and behavior', async (st) => {
    ...
    // Mock REST calls
    st.mock.method(githubApi.octokit.rest.pulls, 'get', () =>
      Promise.reject(new Error('Not found'))
    );
    st.mock.method(githubApi.octokit.rest.issues, 'get', () =>
      Promise.reject(new Error('Not found'))
    );
