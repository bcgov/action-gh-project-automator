...
test('Watermark Utility - GHA Cache Integration', async (t) => {
  ...
  await t.test('should return null when no watermark exists for project', async () => {
    mockCacheData = {};
    const watermark = await getWatermark('my-project', {
      cache: mockCache,
      fs: mockFs,
      log: mockLog,
    });
  ...
  await t.test('should handle project ID sanitization correctly', async () => {
    ...
    await saveWatermark(specialProjectId, timestamp, {
      cache: mockCache,
      fs: mockFs,
      log: mockLog,
    });

    const watermark = await getWatermark(specialProjectId, {
      cache: mockCache,
      fs: mockFs,
      log: mockLog,
    });
    ...
    assert.ok(
      prefixes[0].includes('org_projects_123'),
      `Prefix ${prefixes[0]} should be sanitized`
    );
  });
  ...
  await t.test('should return null if restored timestamp is invalid', async () => {
    ...
    const watermark = await getWatermark('bad-data', {
      cache: mockCache,
      fs: corruptedFs,
      log: mockLog,
    });
