...
describe('Existing item detection', () => {
  test('should verify detection criteria for issues and PRs', () => {
    ...
    assert.strictEqual(
      shouldAddItemToProject(itemInRepo, monitoredUser, monitoredRepos),
      true,
      'Should add items from monitored repos'
    );
    ...
    assert.strictEqual(
      shouldAddItemToProject(prByAuthor, monitoredUser, monitoredRepos),
      true,
      'Should add PRs authored by monitored user'
    );
    ...
    assert.strictEqual(
      shouldAddItemToProject(assignedIssue, monitoredUser, monitoredRepos),
      true,
      'Should add issues assigned to monitored user'
    );
    ...
    assert.strictEqual(
      shouldAddItemToProject(otherItem, monitoredUser, monitoredRepos),
      false,
      'Should not add items that do not match criteria'
    );
  });
});
